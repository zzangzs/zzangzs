...
class KDTree
{
  ...
  private:
    ...
    Point<Dim> select(vector<Point<Dim>>& list, int dim, int left, int right, int k);
    KDTree<Dim>::KDTreeNode* buildTree(vector<Point<Dim>> &points, int dim, int left, int right);
